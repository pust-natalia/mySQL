DROP DATABASE IF EXISTS vk; -- ДЗ 3 В КОНЦЕ ФАЙЛА
...
У поста должен быть id, id создателя, контент (пока что только текст), время создания
и время обновления
*/
...
/*
Создание таблицы с лайками на постах
Нужно хранить пост, на который поставлен лайк, пользователя, который поставил лайк
*/

CREATE TABLE likes_on_posts (
  post_id BIGINT UNSIGNED NOT NULL,
  user_id BIGINT UNSIGNED NOT NULL,
  PRIMARY KEY (post_id, user_id), -- составной PK нужен потому что одному лайку соответствует пользователь и пост
...
/*
Создание таблицы с комментариями на постах
В таблице должен быть пост, к которому оставили комментарий, пользователь, оставивший комментарий, 
текст комментария, дата создания, дата обновления. id наверное тоже не нужен
*/
